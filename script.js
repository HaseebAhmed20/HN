...
// Formspree endpoint (replace with your real form ID, e.g., https://formspree.io/f/abcde123)
const FORMSPREE_ENDPOINT = 'https://formspree.io/f/xzzdjloo';
...
function showContactSuccess(message) {
    if (!contactSuccessEl) return;
    const textEl = contactSuccessEl.querySelector('.success-text');
    if (textEl && message) textEl.textContent = message;
    contactSuccessEl.style.display = 'flex';
    // trigger animation frame to ensure transition applies after display change
    requestAnimationFrame(() => contactSuccessEl.classList.add('show'));
    // auto-hide after 4 seconds
    setTimeout(() => {
        contactSuccessEl.classList.remove('show');
        setTimeout(() => { contactSuccessEl.style.display = 'none'; }, 300);
    }, 4000);
}

function showContactError(message) {
    if (!contactErrorEl) return;
    const textEl = contactErrorEl.querySelector('.error-text');
    if (textEl && message) textEl.textContent = message;
    contactErrorEl.style.display = 'flex';
    requestAnimationFrame(() => contactErrorEl.classList.add('show'));
    setTimeout(() => {
        contactErrorEl.classList.remove('show');
        setTimeout(() => { contactErrorEl.style.display = 'none'; }, 300);
    }, 5000);
}
...
contactForm.addEventListener('submit', async (e) => {
    ...
    // Ensure subject is visible in Formspree email
    const userSubject = (formData.get('subject') || '').toString().trim();
    const userEmail = (formData.get('email') || '').toString().trim();
    const originalMessage = (formData.get('message') || '').toString();

    // Set dynamic email subject header for Formspree
    formData.set('_subject', `Hainova Contact: ${userSubject || 'New Message'}`);

    // Prepend subject into message body so it's clearly visible in the email content
    formData.set('message', `Subject: ${userSubject || '(none)'}\n\n${originalMessage}`);

    // Set reply-to so you can reply directly from the email client
    formData.set('_replyto', userEmail);

    // Hide any previous banners
    if (contactSuccessEl) { contactSuccessEl.classList.remove('show'); contactSuccessEl.style.display = 'none'; }
    if (contactErrorEl) { contactErrorEl.classList.remove('show'); contactErrorEl.style.display = 'none'; }
    ...
    try {
        ...
        if (res.ok) {
            contactForm.reset();
            showContactSuccess('Thanks! Your message has been sent.');
        } else {
            // Try to read error details
            let errMsg = 'Sorry, there was an issue sending your message. Please try again.';
            try {
...
const whatsappNumber = '923114137651';
const whatsappMessage = encodeURIComponent('Hi! I would like to know more about Hainova Solutions services.');

if (whatsappLink) {
    // Ensure anchor has a default desktop URL (prevents popup blockers)
    whatsappLink.setAttribute('href', `https://wa.me/${whatsappNumber}?text=${whatsappMessage}`);

    whatsappLink.addEventListener('click', (e) => {
        // On mobile, prefer deep link to the app with web fallback
        if (isMobile) {
            e.preventDefault();
            const deepLink = `whatsapp://send?phone=${whatsappNumber}&text=${whatsappMessage}`;
            const webLink = `https://wa.me/${whatsappNumber}?text=${whatsappMessage}`;
